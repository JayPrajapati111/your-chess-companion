...
export const ChessBoard = ({ onGameEnd }: ChessBoardProps) => {
      ...
      <div className="flex items-center gap-4">
        <div
          className={`px-4 py-2 rounded-lg font-semibold ${
            currentTurn === "white" ? "bg-foreground text-background" : "bg-card text-foreground"
          }`}
        >
        ...
        {gameStatus === "stalemate" && (
          <span className="text-muted-foreground font-bold">STALEMATE! It's a draw!</span>
        )}
      </div>
                    ...
                    {piece && (
                      <span
                        className={`text-3xl md:text-5xl select-none transition-transform duration-150 ${
                          selected ? "scale-110" : ""
                        } ${piece.color === "white" ? "drop-shadow-md" : "drop-shadow-lg"}`}
                        style={{
                          filter: piece.color === "white" ? "drop-shadow(1px 1px 2px rgba(0,0,0,0.5))" : "none",
                        }}
                      >
